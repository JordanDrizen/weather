...
def submit_weather_view():
	...
	if weather_view.get() == "current":
		...
		def submit():
			...
			try:
				...
				if exact_entry_checkbox_var.get() == 1:
					exact_entry_label.config(text = "Exact Entry: " + response_exact_entry)
				...
				if current_temp_checkbox_var.get() == 1:
					...
					if unit.get() == "m":
						...
						current_temp_label.config(text = "Current Temperature: " + str(response_current_temp) + degree_symbol + temperature_symbol)
					elif unit.get() == "s":
						temperature_symbol = "K"
						current_temp_label.config(text = "Current Temperature: " + str(response_current_temp) + temperature_symbol)
					elif unit.get() == "f":
						temperature_symbol = "F"
						current_temp_label.config(text = "Current Temperature: " + str(response_current_temp) + degree_symbol + temperature_symbol)
				
				if current_time_checkbox_var.get() == 1:
					current_time_label.config(text = "Current Time: " + response_current_time)
					current_time_label.grid(row = 2, column = 1)

				if weather_description_checkbox_var.get() == 1:
					weather_description_label.config(text = "Weather Description: " + str(response_weather_description))
					weather_description_label.grid(row = 1, column = 2)

				if wind_speed_checkbox_var.get() == 1:
					if unit.get() == "m" or unit.get() == "s":
						wind_speed_label.config(text = "Wind Speed: " + str(response_wind_speed) + "km/h")
					elif unit.get() == "f":
						wind_speed_label.config(text = "Wind Speed: " + str(response_wind_speed) + "mph")
					wind_speed_label.grid(row = 2, column = 2)

				if humidity_checkbox_var.get() == 1:
					humidity_label.config(text = "Humidity: " + str(response_humidity) + "%")
					humidity_label.grid(row = 3, column = 2)

				if feels_like_checkbox_var.get() == 1:
					feels_like_label.config(text = "Feels Like: " + str(response_feels_like) + degree_symbol + temperature_symbol)
					feels_like_label.grid(row = 4, column = 0)

				if visibility_checkbox_var.get() == 1:
					if unit.get() == "m" or unit.get() == "s":
						visibility_label.config(text = "Visibility: " + str(response_visibility) + "km")
					elif unit.get() == "f":
						visibility_label.config(text = "Visibility: " + str(response_visibility) + " miles")
		...
		scientific_button = Radiobutton(root, text = "Scientific", variable = unit, value = "s")
		fahrenheit_button = Radiobutton(root, text = "Fahrenheit", variable = unit, value = "f")
		...
		#Checkboxes
		entry_type_checkbox = Checkbutton(root, text = "Entry Type", variable = entry_type_checkbox_var)
		exact_entry_checkbox = Checkbutton(root, text = "Exact Entry", variable = exact_entry_checkbox_var)
		latitude_checkbox = Checkbutton(root, text = "Latitude", variable = latitude_checkbox_var)
		longitude_checkbox = Checkbutton(root, text = "Longitude", variable = longitude_checkbox_var)
		city_checkbox = Checkbutton(root, text = "City", variable = city_checkbox_var)
		current_temp_checkbox = Checkbutton(root, text = "Current Temperature", variable = current_temp_checkbox_var)
		current_time_checkbox = Checkbutton(root, text = "Current Time", variable = current_time_checkbox_var)
		weather_description_checkbox = Checkbutton(root, text = "Weather Description", variable = weather_description_checkbox_var)
		wind_speed_checkbox = Checkbutton(root, text = "Wind Speed", variable = wind_speed_checkbox_var)
		humidity_checkbox = Checkbutton(root, text = "Humidity", variable = humidity_checkbox_var)
		feels_like_checkbox = Checkbutton(root, text = "Feels Like", variable = feels_like_checkbox_var)
		visibility_checkbox = Checkbutton(root, text = "Visibility", variable = visibility_checkbox_var)
		select_all_checkbox = Checkbutton(root, text = "Select All", variable = select_all_checkbox_var)
		...
		historical_label = Label(root, text = "You've chosen historical weather!", font = 'Helvetica').pack()


weather_view = StringVar()
weather_view.set("current")
current_weather_button = Radiobutton(root, text = "Current Weather", variable = weather_view, value = "current")
historical_weather_button = Radiobutton(root, text = "Historical Weather", variable = weather_view, value = "historical")
